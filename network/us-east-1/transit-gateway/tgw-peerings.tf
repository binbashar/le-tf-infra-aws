...
resource "aws_ec2_transit_gateway_route" "network-dr" {
  ...
  for_each = { for k, v in local.network-dr-vpcs : k => v if var.enable_tgw && var.enable_tgw_multi_region && try(data.terraform_remote_state.tgw-dr.outputs.tgw_id != null, false) }
...
resource "aws_ec2_transit_gateway_route" "shared-dr" {
  ...
  for_each = { for k, v in local.shared-dr-vpcs : k => v if var.enable_tgw && var.enable_tgw_multi_region && try(data.terraform_remote_state.tgw-dr.outputs.tgw_id != null, false) }
...
resource "aws_ec2_transit_gateway_route" "apps-devstg-dr" {
  ...
  for_each = { for k, v in local.apps-devstg-dr-vpcs : k => v if var.enable_tgw && var.enable_tgw_multi_region && try(data.terraform_remote_state.tgw-dr.outputs.tgw_id != null, false) }
