...
The FULL format is defined in `.claude/docs/output-formats.md` (single source of truth). Locally this skill prints the complete output to the terminal unfiltered — no cap, no artifact. Use `/tf-plan` instead if you only need to see what will change (delta format).
