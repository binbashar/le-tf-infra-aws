...
### Subtasks:
- [x] T-001.1: Create layer directory `data-science/us-east-1/bedrock-agent-kyb`
- [x] T-001.2: Create `config.tf` with providers (aws, awscc, archive)
- [x] T-001.3: Create `variables.tf` with standard layer variables
- [x] T-001.4: Create `locals.tf` with naming conventions and tags
- [x] T-001.5: Create `outputs.tf` with layer outputs structure
- [x] T-001.6: Create `common-variables.tf` symlink
...
### Subtasks:
- [x] T-002.1: Create `s3.tf` file
- [x] T-002.2: Implement input bucket with EventBridge notifications
- [x] T-002.3: Implement processing bucket with EventBridge notifications
- [x] T-002.4: Implement output bucket with standard configurations
- [x] T-002.5: Add bucket policies and encryption settings
- [x] T-002.6: Configure lifecycle management
- [x] T-002.7: Add S3 bucket outputs to `outputs.tf`
...
### Subtasks:
- [x] T-003.1: Create `bedrock.tf` file
- [x] T-003.2: Implement BDA project resource with standard output
- [x] T-003.3: Configure BDA project without custom blueprint
...
**Detailed Plan**: See [t-005-plan.md](./tasks/t-005-plan.md)

### Subtasks:
- [x] T-005.1: Create `lambda.tf` file with archive data sources
- [x] T-005.2: Create Lambda source directories with minimal handlers
- [x] T-005.3: Add Lambda function resources to `lambda.tf`
- [x] T-005.4: Add CloudWatch Log Groups to `lambda.tf`
- [x] T-005.5: Create `iam.tf` with Lambda execution roles
- [x] T-005.6: Add Lambda permissions for service invocation
- [x] T-005.7: Add Lambda variables to `variables.tf`
- [x] T-005.8: Add Lambda outputs to `outputs.tf`
...
## [x] T-009: Bedrock Agent Configuration
...
### Subtasks:
- [x] T-009.1: Create OpenAPI schemas for action groups (get_documents.yaml, save_document.yaml)
- [x] T-009.2: Create IAM role for Bedrock Agent with model and Lambda permissions
- [x] T-009.3: Add Bedrock Agent resource to `bedrock.tf` with 2 action groups (uses separate resources)
- [x] T-009.4: Create Agent Alias resource for versioning
- [x] T-009.5: Update Agent Invoker Lambda environment variables with real agent IDs
- [x] T-009.6: Add Bedrock Agent outputs to `outputs.tf`
