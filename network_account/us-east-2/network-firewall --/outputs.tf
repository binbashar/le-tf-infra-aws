...
output "network_firewall_status" {
  ...
  value       = var.enable_network_firewall ? module.firewall[0].network_firewall_status : []
}

output "sync_states" {
  description = "Set of subnets configured for use by the firewall."
  value       = var.enable_network_firewall ? module.firewall[0].network_firewall_status.*.sync_states : []
}

output "network_firewall_subnet_id_endpoint_id" {
  description = "Map of endpoint_id per subnet_id"
  value       = var.enable_network_firewall ? { for v in module.firewall[0].network_firewall_status[0]["sync_states"].*.attachment : v[0]["subnet_id"] => v[0]["endpoint_id"] } : {}
}
