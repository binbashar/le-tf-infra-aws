resource "kubernetes_namespace" "monitoring_metrics" {
  count = var.scaling.cluster_autoscaling.enabled || var.scaling.hpa.enabled || var.scaling.vpa.enabled || var.goldilocks.enabled ? 1 : 0
...
resource "kubernetes_namespace" "monitoring_other" {
  count = var.datadog_agent.enabled || var.uptime_kuma.enabled || var.gatus.enabled ? 1 : 0
