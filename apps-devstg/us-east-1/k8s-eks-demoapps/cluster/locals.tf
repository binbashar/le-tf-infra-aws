locals {
  ...
  # It used to be hardcoded here and had silently gone stale: the suffix on the
  # mapped role no longer matched the live one, so that aws-auth entry had been
  # granting nothing. Resolve it instead of pinning it. (The suffixes themselves
  # are deliberately not written down -- this repo is public, and a live SSO role
  # name is half of a usable principal ARN. Read them with
