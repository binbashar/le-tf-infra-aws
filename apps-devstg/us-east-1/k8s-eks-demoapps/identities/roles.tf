...
module "role_certmanager" {
  ...
  create_role  = true
  role_name    = "${local.environment}-demoapps-certmanager"
...
module "role_externaldns_private" {
  ...
  create_role  = true
  role_name    = "${local.environment}-demoapps-externaldns-private"
  provider_url = replace(data.terraform_remote_state.apps-devstg-eks-demoapps-cluster.outputs.cluster_oidc_issuer_url, "https://", "")
...
module "role_externaldns_public" {
  ...
  create_role  = true
  role_name    = "${local.environment}-demoapps-externaldns-public"
  provider_url = replace(data.terraform_remote_state.apps-devstg-eks-demoapps-cluster.outputs.cluster_oidc_issuer_url, "https://", "")
...
module "role_cluster_autoscaler" {
  ...
  create_role  = true
  role_name    = "${local.environment}-demoapps-cluster-autoscaler"
  provider_url = replace(data.terraform_remote_state.apps-devstg-eks-demoapps-cluster.outputs.cluster_oidc_issuer_url, "https://", "")
