...
module "role_externaldns_public" {
  ...
  create_role  = true
  role_name    = "${local.environment}-${local.prefix}-externaldns-public"
  provider_url = replace(data.terraform_remote_state.cluster.outputs.cluster_oidc_issuer_url, "https://", "")

  role_policy_arns = [
    aws_iam_policy.externaldns_public_binbash_com_ar.arn
  ]
...
resource "aws_iam_policy" "externaldns_public_binbash_com_ar" {
  provider    = aws.shared
  name        = "${local.environment}-${local.prefix}-externaldns-public-binbash.com.ar"
...
module "role_externaldns_private" {
  ...
  create_role  = true
  role_name    = "${local.environment}-${local.prefix}-externaldns-private"
  provider_url = replace(data.terraform_remote_state.cluster.outputs.cluster_oidc_issuer_url, "https://", "")
...
resource "aws_iam_policy" "externaldns_private_aws_binbash_com_ar" {
  provider    = aws.shared
  name        = "${local.environment}-${local.prefix}-externaldns-private-aws.binbash.com.ar"
