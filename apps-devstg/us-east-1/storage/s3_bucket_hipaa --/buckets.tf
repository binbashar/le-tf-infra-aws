...
module "customers_buckets" {
  ...
  logging = {
    target_bucket = module.customers_logging_buckets[each.key].s3_bucket_id
    target_prefix = "logs/"
  }
...
module "customers_logging_buckets" {
  source = "github.com/binbashar/terraform-aws-s3-bucket.git?ref=v2.4.0"

  for_each = toset(var.customers)

  bucket                                = "${var.project}-${var.prefix}-customer-${each.key}-files-logs"
