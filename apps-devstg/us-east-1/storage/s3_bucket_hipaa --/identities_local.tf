...
module "customers_roles" {
  ...
  custom_role_policy_arns = [aws_iam_policy.customers_roles_policy[each.key].arn]
}

#
# The role will be attached t o a policy that enables access to a given S3 buclet.
#
resource "aws_iam_policy" "customers_roles_policy" {
  for_each = toset(var.customers)
  ...
  policy      = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Resource": "${module.customers_buckets[each.key].s3_bucket_arn}"
    },
    {
      "Action": [
        "s3:GetObject",
