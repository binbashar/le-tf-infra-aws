...
resource "null_resource" "associate_apps_devstg_eks_vpc_with_private_remote_hosted_zone" {
  ...
  provisioner "local-exec" {
    ...
    command     = "aws route53 associate-vpc-with-hosted-zone --profile ${var.profile} --hosted-zone-id ${data.terraform_remote_state.dns-shared.outputs.aws_internal_zone_id[0]} --vpc VPCRegion=${var.region},VPCId=${module.vpc-eks.vpc_id}"
  }
}
