...
#
module "config_logs" {
  source = "github.com/binbashar/terraform-aws-logs.git?ref=v11.0.0"

  s3_bucket_name          = "${var.project}-${var.environment}-awsconfig-dr-logs"
  ...
  config_logs_prefix      = "${var.project}-${var.environment}-awsconfig-dr"
...
module "terraform-aws-config" {
  ...
  ami_required_tag_key_value = "ApprovedAMI:true-dr"
}
