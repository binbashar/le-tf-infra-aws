...
output "cluster_master_azs" {
  ...
  value = [
    data.terraform_remote_state.vpc.outputs.availability_zones[0],
    data.terraform_remote_state.vpc.outputs.availability_zones[1],
    data.terraform_remote_state.vpc.outputs.availability_zones[2]
  ]
}
