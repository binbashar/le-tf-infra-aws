...
resource "aws_vpc_peering_connection_accepter" "with_dev_eks" {
  ...
  vpc_peering_connection_id = data.terraform_remote_state.vpc-dev.outputs.vpc_peering_id_dev_eks_with_shared
  auto_accept               = true
...
resource "aws_route" "priv_route_table_1_to_dev_eks_vpc" {
  ...
  destination_cidr_block    = data.terraform_remote_state.vpc-eks.outputs.vpc_cidr_block
  vpc_peering_connection_id = data.terraform_remote_state.vpc-dev.outputs.vpc_peering_id_dev_eks_with_shared
}
