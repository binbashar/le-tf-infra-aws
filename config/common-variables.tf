...
locals {
  ...
  # Try to find the current region from path; fall back to region_primary
  current_region = try([for r in local.regions : r if can(regex(r, path.cwd))][0], var.region_primary)
