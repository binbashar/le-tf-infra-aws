...
#
# NOT created here: app-aws-startups-accelerate already owns the account-wide
# provider for this issuer, and a second one fails with EntityAlreadyExists.
# var.create_github_oidc_provider therefore defaults to false in this layer (it
# defaults to true in that one), so the count below is 0 and the data source
# looks the existing provider up by URL. See issue #1081 for moving ownership
# to a dedicated identities layer.
#
...
resource "aws_iam_role" "github_actions_deploy" {
  name               = "${var.project}-${var.environment}-${local.app_name}-github-actions-oidc"
...
data "aws_iam_policy_document" "github_actions_deploy" {
  statement {
    ...
    resources = [module.binbash_web.s3_bucket_arn]
  }
  ...
  statement {
    ...
    resources = [module.binbash_web.cf_arn]
  }
}
