...
variable "staging_basic_auth_username" {
  description = <<-EOT
    ...
    password is secret — it is generated by random_password and read with
  ...
  validation {
    condition     = can(regex("^[A-Za-z0-9._-]+$", var.staging_basic_auth_username))
    error_message = "staging_basic_auth_username must not contain ':' or whitespace — it is base64-encoded into an HTTP Basic Authorization header as 'user:password'."
  }
}
