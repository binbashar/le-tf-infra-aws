...
variable "create_github_oidc_provider" {
  description = <<-EOT
    ...
    Defaults to false here — unlike app-aws-startups-accelerate, which defaults
    to true because it is the layer that actually owns the account-wide
    aws_iam_openid_connect_provider for token.actions.githubusercontent.com.
    There is one such provider per AWS account, so creating a second one fails
    with EntityAlreadyExists; with false the existing provider is looked up by
    URL instead. Flip only if provider ownership moves out of that layer —
    see issue #1081.
  EOT
  type        = bool
  default     = false
...
variable "dns_cutover_enabled" {
  description = <<-EOT
    Whether to create the Route 53 alias records that point binbash.co and
