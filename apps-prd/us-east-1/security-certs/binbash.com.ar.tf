...
resource "aws_route53_record" "cert_validation_binbash_com_ar" {
  ...
  for_each = {
    for dvo in local.domain_validation_options_binbash_com_ar : dvo.domain_name => {
...
locals {
  ...
  domains_binbash_com_ar = aws_acm_certificate.binbash_com_ar.domain_validation_options

  # Get domain names as a list to be able to comapre records like domain-org to *.domain.org
  domains_names_binbash_com_ar = [
    for domain in local.domains_binbash_com_ar :
    domain.domain_name
  ]

  # Avoid domain-org alike records
  domain_validation_options_binbash_com_ar = [
    for domain in local.domains_binbash_com_ar :
    domain if !contains(local.domains_names_binbash_com_ar, "*.${domain.domain_name}")
  ]
}
