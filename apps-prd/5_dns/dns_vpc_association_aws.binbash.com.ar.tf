...
#====================================================================#
# HOSTED ZONE: aws.binbash.com.ar from Shared to Apps Prd Account    #
#====================================================================#
// eg:
...
#
# Accept association between Apps Prd VPC and aws.binbash.com.ar private hosted zone
#
resource "null_resource" "associate_apps_prd_vpc_with_private_remote_hosted_zone" {
  count = var.vpc_shared_dns_assoc == true ? 1 : 0

  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command     = "aws route53 associate-vpc-with-hosted-zone --profile ${var.profile} --hosted-zone-id ${data.terraform_remote_state.dns-shared.outputs.aws_internal_zone_id[0]} --vpc VPCRegion=${var.region},VPCId=${data.terraform_remote_state.vpc-apps-prd.outputs.vpc_id}"
  }
}
