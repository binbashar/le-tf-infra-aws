...
Secrets referenced by the Terraform Plan workflow in this repository. (The apply
and Claude assistant/review workflows arrive in later staged PRs; this inventory
will grow with them.)
...
|--------|---------|------|---------|
| `CLAUDE_CODE_OAUTH_TOKEN` | terraform-plan.yml | Medium | OAuth token for Claude Code Action — authenticates AI plan analysis |
| `AWS_ACCESS_KEY_ID` | terraform-plan.yml | **High** | Static IAM access key for DeployMaster role — used to run plans |
| `AWS_SECRET_ACCESS_KEY` | terraform-plan.yml | **High** | Static IAM secret key paired with access key |
| `AWS_MANAGEMENT_ACCOUNT_ID` | terraform-plan.yml | Low | Management account ID for assume-role ARN construction |
| `AWS_SECURITY_ACCOUNT_ID` | terraform-plan.yml | Low | Security account ID for assume-role ARN construction |
| `AWS_SHARED_ACCOUNT_ID` | terraform-plan.yml | Low | Shared account ID for assume-role ARN construction |
| `AWS_NETWORK_ACCOUNT_ID` | terraform-plan.yml | Low | Network account ID for assume-role ARN construction |
| `AWS_DEVSTG_ACCOUNT_ID` | terraform-plan.yml | Low | DevStg account ID for assume-role ARN construction |
| `AWS_PRD_ACCOUNT_ID` | terraform-plan.yml | Low | Production account ID for assume-role ARN construction |
| `AWS_DATA_SCIENCE_ACCOUNT_ID` | terraform-plan.yml | Low | Data Science account ID for assume-role ARN construction |
| `GITHUB_TOKEN` | terraform-plan.yml | Low | Auto-rotated per workflow run — scoped to repo permissions |
...
- Only the plan job uses these credentials
...
- The migration justifies adding `id-token: write` to the plan job. This workflow
  does **not** grant `id-token: write` today (nothing consumes an OIDC token until
  this migration lands); add it as part of the migration.
