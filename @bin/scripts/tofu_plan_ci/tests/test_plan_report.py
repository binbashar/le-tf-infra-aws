...
class PlanReportTests(unittest.TestCase):
    ...
    def test_live_failure_exposes_only_stage_and_action(self):
        with tempfile.TemporaryDirectory() as directory:
            out = Path(directory)
            main(["live", "--layer", "apps-devstg/global/cli-test-layer", "--init-exit", "0", "--validate-exit", "0", "--plan-exit", "1", "--failure-stage", "plan", "--denied-action", "dynamodb:PutItem", "--failure-diagnostic", "Error acquiring the state lock", "--failure-location", "roles.tf:12", "--out-dir", str(out)])
            result = json.loads((out / "result.json").read_text())
            self.assertEqual(result["failure"], {"stage": "plan", "denied_action": "dynamodb:PutItem", "diagnostic": "Error acquiring the state lock", "common_tfvars_shape": None, "location": "roles.tf:12"})
